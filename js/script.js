...
const posts = [
  ...
  },
  {
    ...
    "profile": "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/834.jpg",
...
const writeBtn = document.querySelector('.fa-pen-to-square').parentElement;
writeBtn.addEventListener('click', toggleForm);

const submitBtn = formContainer.children[0].children[3];
...
function postArticle(e) {
  ...
  newArticle.id = posts.length;
  let newTitle = document.getElementById('title').value;
  newArticle.title = newTitle;
  let newAuthor = document.getElementById('author').value;
  newArticle.author = newAuthor;
  let newDate = new Date();
  newArticle.date = newDate.toISOString();
  newArticle.profile = new URL("http://localhost:5501/images/default.jpeg");
...
function showArticle(e) {
  if (e.target.closest('.card')) {
    ...
    let cardHeader = document.createElement('div');
    cardHeader.classList.add("article-header");

    let profilePic = document.createElement('img');
    profilePic.src = targetArticle.profile;
    profilePic.alt = "profile picture";
    profilePic.classList.add("avatar");
    ...
    let headText = document.createElement('div');
    let postDate = new Date(targetArticle.date);

    headText.innerText = targetArticle.author + " • " + weekday[postDate.getDay()] + " " + monthNames[postDate.getMonth()] + " " + postDate.getDate() + " " + postDate.getFullYear();

    cardHeader.appendChild(headText);
    articleWrapper.appendChild(cardHeader);

    //create article-body

    let text = document.createElement('p');
    text.classList.add('article-body');
    text.innerText = targetArticle.content;
